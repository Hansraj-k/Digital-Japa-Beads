...
function isSmartwatch() {
    return (window.matchMedia("(max-width: 300px) and (max-height: 300px)").matches) || 
           (window.matchMedia("(max-device-width: 300px) and (max-device-height: 300px)").matches);
}

// Adjust UI for smartwatch
function adjustForSmartwatch() {
    if (isSmartwatch()) {
        // Reduce button size for smartwatch
        updateButtonSize(70);
        
        // Simplify the UI
        document.querySelector('.streak-calendar').style.display = 'none';
        document.querySelector('.install-container').style.display = 'none';
        
        // Adjust vibration durations for watch haptics
        countVibrationSlider.value = 30;
        completeVibrationSlider.value = 500;
        updateSettingsUI();
        
        // Save these settings
        vibrationSettings.countVibrationDuration = 30;
        vibrationSettings.completeVibrationDuration = 500;
        saveSettingsToStorage();
    }
}
...
window.addEventListener('load', function() {
    ...
    // Add touch event for better interaction on watches
    if (isSmartwatch()) {
        const countBtn = document.getElementById('count-btn');
        
        // Add touch feedback
        countBtn.addEventListener('touchstart', function() {
            this.style.transform = `scale(${currentButtonSize/100 * 0.9})`;
        });
        
        countBtn.addEventListener('touchend', function() {
            this.style.transform = `scale(${currentButtonSize/100})`;
        });
    }
});

// Modify the vibration patterns for smartwatches
function vibrate(duration) {
    if (!navigator.vibrate) return;
    
    if (isSmartwatch()) {
        // Shorter, more distinct vibrations for watches
        navigator.vibrate(duration > 100 ? [100, 50, 100] : duration);
    } else {
        navigator.vibrate(duration);
    }
}

// Update all vibration calls to use this new function
