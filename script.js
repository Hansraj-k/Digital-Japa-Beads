...
// Get elements for the image and text popups
let roundImage = document.getElementById('round-image');
let circleText = document.getElementById('circle-text');
let popupChangeImage = document.getElementById('popup-change-image');
let popupUploadImage = document.getElementById('popup-upload-image');
let popupChangeText = document.getElementById('popup-change-text');

// Right-click or long-click to change image/text
roundImage.addEventListener('contextmenu', (event) => {
    event.preventDefault(); // Prevent right-click menu
    popupChangeImage.style.display = 'block'; // Show the popup for changing image/text
});
...
roundImage.addEventListener('touchstart', (event) => {
    ...
    const touchEndHandler = () => {
        touchDuration = Date.now() - touchStart;
        if (touchDuration >= 1000) { // Long press duration
            popupChangeImage.style.display = 'block'; // Show the popup for changing image/text
        }
        roundImage.removeEventListener('touchend', touchEndHandler); // Remove event after touch ends
    };

    roundImage.addEventListener('touchend', touchEndHandler);
});

// Button to change image
document.getElementById('change-image-btn').addEventListener('click', () => {
    popupChangeImage.style.display = 'none'; // Close the change image popup
    popupUploadImage.style.display = 'block'; // Show the upload image popup
});

// Button to change text
document.getElementById('change-text-btn').addEventListener('click', () => {
    popupChangeImage.style.display = 'none'; // Close the change text popup
    popupChangeText.style.display = 'block'; // Show the change text popup
});
...
document.getElementById('upload-image').addEventListener('change', (event) => {
    ...
    if (file) {
        ...
        reader.onload = function(e) {
            roundImage.src = e.target.result; // Set the new image
        };
...
document.getElementById('confirm-upload-image').addEventListener('click', () => {
    popupUploadImage.style.display = 'none'; // Close the popup
});
...
// Change text
document.getElementById('confirm-change-text').addEventListener('click', () => {
    const newText = document.getElementById('new-text').value;
    if (newText) {
        circleText.innerHTML = newText; // Update the text in the circle
    }
    popupChangeText.style.display = 'none'; // Close the popup
});

// Cancel change text
document.getElementById('cancel-change-text').addEventListener('click', () => {
    popupChangeText.style.display = 'none'; // Close the popup
});

// Close the change image popup
document.getElementById('close-popup').addEventListener('click', () => {
    popupChangeImage.style.display = 'none';
});
